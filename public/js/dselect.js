...
function dselect(el, option = {}) {
  ...
  const searchInput = search ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="dselectSearch(event, this, '${classElement}', '${classToggler}', ${creatable})" type="text" class="form-control" placeholder="Поиск" autofocus>` : ''
  ...
  function itemTags(options) {
    ...
    for (const option of options) {
      if (option.tagName === 'OPTGROUP') {
        ...
        let disabled = el.multiple && option.selected ? ' disabled' : ''
        let hidden = el.multiple && option.hidden ? ' hidden' : ''
        const value = option.value
        const text = option.textContent
        
        disabled = option.disabled ? 'disabled' : disabled
        hidden = option.hidden ? 'hidden' : hidden
    ...
    <div class="dropdown ${classElement} ${additionalClass}">
      ...
      <div class="dropdown-menu">
        <div class="d-flex flex-column">
          ...
          <div class="${classNoResults} d-none">Поиск не дал результата</div>
        </div>
      </div>
      ${clearBtn}
    </div>
